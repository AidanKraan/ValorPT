    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 500, background: T.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 24, animation: "fadeIn 0.3s ease" }}>
      ...
      <div style={{ animation: "celebrationScale 0.6s cubic-bezier(0.16,1,0.3,1) forwards", position: "relative", zIndex: 2 }}>
        <svg width="140" height="140" viewBox="0 0 140 140">
          <defs>
            <linearGradient id="celebGrad" x1="0" y1="0" x2="140" y2="140" gradientUnits="userSpaceOnUse">
...
function AnimNum({ value, suffix = "", decimals = 0 }) {
  ...
  useEffect(() => {
    ...
    const tick = now => {
      ...
      setDisp(+(start + (end - start) * ease).toFixed(decimals));
...
function Chart({ data, color = T.green, height = 120, label = "c", minVal, maxVal }) {
  ...
  const pad = { t: 12, b: 28, l: 34, r: 10 };
  const cw = W - pad.l - pad.r, ch = H - pad.t - pad.b;
  const mn = minVal ?? Math.min(...data) - 5;
  const mx = maxVal ?? Math.max(...data) + 5;
  ...
  return (
    <div onClick={onClick} className={hover ? "card-hover" : ""}
      style={{
        ...
        boxShadow: glow ? `0 0 28px ${T.greenGlow}, inset 0 1px 0 rgba(255,255,255,0.06)` : "inset 0 1px 0 rgba(255,255,255,0.04)",
...
function Btn({ children, onClick, variant = "primary", style = {}, disabled = false, icon: IconCmp }) {
  ...
  const V = {
    primary:   { background: `linear-gradient(135deg, ${T.green} 0%, ${T.greenDeep} 100%)`, color: "#fff", boxShadow: `0 4px 24px ${T.greenGlow}` },
    secondary: { background: "transparent", color: T.green, border: `1.5px solid ${T.green}` },
    ghost:     { background: T.grayD, color: T.gray, border: `1px solid ${T.border}` },
    danger:    { background: `linear-gradient(135deg, ${T.red}, #8B0000)`, color: "#fff" },
  };
  return (
    ...
    <div style={{ background: "linear-gradient(180deg, #0D2818 0%, rgba(13,40,24,0.5) 60%, transparent 100%)", padding: "24px 20px 20px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative" }}>
        <div>
          ...
          {subtitle && <p style={{ color: T.gray, fontSize: 13, marginTop: 5, fontFamily: font }}>{subtitle}</p>}
        </div>
        {right}
      </div>
    </div>
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const bone = (a, b) => {
        ...
        gr.addColorStop(0, "rgba(0,200,83,0.75)"); gr.addColorStop(1, "rgba(0,200,83,0.55)");
        ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y);
        ctx.strokeStyle = gr; ctx.lineWidth = 2.5; ctx.lineCap = "round"; ctx.stroke();
      };
      ...
      const kL = j.kneeL, hip = j.hipL, ank = j.ankleL;
      const a1 = Math.atan2(hip.y - kL.y, hip.x - kL.x), a2 = Math.atan2(ank.y - kL.y, ank.x - kL.x);
      ctx.beginPath(); ctx.arc(kL.x, kL.y, 24, a1, a2);
      ctx.strokeStyle = "rgba(255,149,0,0.8)"; ctx.lineWidth = 2; ctx.stroke();
      ctx.fillStyle = "#FF9500"; ctx.font = "bold 11px Inter,sans-serif";
      ctx.fillText(`${kneeAngle}°`, kL.x + 18, kL.y - 12);
...
function WebcamFeed({ kneeAngle }) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: 360, borderRadius: 20, overflow: "hidden", border: `1.5px solid ${T.greenBorder}`, boxShadow: `0 0 40px ${T.greenGlow}, 0 0 0 1px rgba(0,200,83,0.1)`, background: T.surface }}>
      ...
      <div style={{ position: "absolute", top: 14, right: 14, display: "flex", alignItems: "center", gap: 6, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(10px)", border: `1px solid ${T.greenBorder}`, borderRadius: 99, padding: "5px 12px" }}>
        <div className="live-pulse" style={{ width: 7, height: 7, borderRadius: "50%", background: T.green }}/>
...
function useMetrics(phase) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      s.current.t += 0.038;
      const tt = s.current.t, maxK = phase === 2 ? 118 : 98;
      ...
      const elapsed = Math.floor((Date.now() - s.current.start) / 1000);
      if (knee > maxK * 0.78 && s.current.phase === "down") s.current.phase = "up";
      else if (knee < 28 && s.current.phase === "up") { s.current.phase = "down"; s.current.count = Math.min(s.current.count + 1, 15); }
      setM({ knee, hip, sym, reps: s.current.count, t: elapsed });
    }, 80);
      ...
      <Card className="fadeUp2" style={{ width: "100%", maxWidth: 400, padding: 32, position: "relative", zIndex: 1 }}>
        <div style={{ marginBottom: 22 }}>
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Enter your name"
            style={{ width: "100%", background: T.surface2, border: `1px solid ${T.border}`, borderRadius: 12, padding: "14px 16px", color: T.white, fontSize: 15, fontFamily: font }}
  ...
  return (
    <div className="fadeUp" style={{ minHeight: "100vh", background: T.bg, color: T.white, paddingBottom: 90 }}>
      <Hero
        ...
        subtitle={`${patient.name} · Phase ${patient.phase}`}
...
function ExerciseSession({ patient, exercise, onComplete, onBack }) {
  ...
  const INSTRS = {
    ...
    "Straight Leg Raises": "Tighten quad, raise leg to 45°. Hold 2s. Lower slowly with full control. Keep opposite knee bent for support.",
    "Long Arc Quad": "Sit at surface edge. Fully straighten leg, hold 5s, lower slowly. Focus on terminal extension and quad activation.",
    "Air Squats (60°)": "Stand feet shoulder-width. Lower to 60° knee flexion. Keep knees aligned over 2nd toe. Control descent and ascent.",
    "Air Squats (30°)": "Stand feet shoulder-width. Lower to 30° knee flexion only. Focus on equal weight distribution and symmetry.",
    "Single Leg Squat 50%": "Stand on operative leg. Lower to half squat, knee over 2nd toe. Control both descent and return equally.",
    "Single Leg Squat Max": "Stand on operative leg. Lower as deep as comfortable maintaining alignment. Use arms for balance only.",
    "Step-Ups": "Step up leading with operative leg. Straighten fully at top before stepping down. Controlled return always.",
      ...
      <div style={{ padding: "20px 16px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <Card>
          ...
          <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
            <defs><linearGradient id="repG" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={T.green} stopOpacity="0.35"/><stop offset="100%" stopColor={T.green} stopOpacity="0.01"/></linearGradient></defs>
            <path d={area} fill="url(#repG)"/>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20, position: "relative" }}>
          <div style={{ position: "relative", width: 160, height: 160, flexShrink: 0 }}>
            <svg width="160" height="160" viewBox="0 0 160 160" style={{ transform: "rotate(-90deg)" }}>
              <defs>
                <linearGradient id="heroRingGrad" x1="0" y1="0" x2="160" y2="160" gradientUnits="userSpaceOnUse">
                  <stop offset="0%" stopColor="#8BC34A"/>
                  <stop offset="100%" stopColor="#1B5E20"/>
                </linearGradient>
              </defs>
      ...
      <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: 14 }}>
        <Card>
          ...
          <div style={{ display: "flex", justifyContent: "space-around" }}>
            <ProgressRing value={108} max={130} size={88} label="108°" sublabel="ROM"/>
            <ProgressRing value={91}  max={100} size={88} label="91%"  sublabel="Symmetry" color={T.orange}/>
          ...
          {sessions.map((s, i) => (
            <div key={s.date} style={{ display: "flex", alignItems: "center", gap: 14, padding: "11px 0", borderBottom: i < sessions.length-1 ? `1px solid ${T.border}` : "none" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: T.gray, fontSize: 12, marginTop: 1 }}>{s.exercises} exercises</div>
              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map(p => (
            <div key={p.id} className="card-hover" onClick={() => onSelectPatient(p)}
              style={{ background: T.glass, backdropFilter: "blur(20px)", border: `1px solid ${p.flagged ? "rgba(255,59,48,0.3)" : T.glassBorder}`, borderRadius: 16, padding: "18px", cursor: "pointer" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 46, height: 46, borderRadius: "50%", background: `linear-gradient(135deg, ${T.greenDeep}, ${T.green})`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 17, color: T.white, flexShrink: 0, boxShadow: `0 0 14px ${T.greenGlow}` }}>
  ...
  return (
    <div className="fadeUp" style={{ minHeight: "100vh", background: T.bg, color: T.white, paddingBottom: 40 }}>
      ...
      <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 14 }}>
        <Card>
          <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
            <div style={{ width: 60, height: 60, borderRadius: 18, background: `linear-gradient(135deg, ${T.greenDeep}, ${T.green})`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 22, color: T.white, flexShrink: 0, boxShadow: `0 0 20px ${T.greenGlow}` }}>
        ...
        <Card>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 10, alignItems: "center" }}>
            ...
            {[T.grayD, `${T.greenDeep}99`, T.green].map((c, i) => <div key={i} style={{ width: 12, height: 12, borderRadius: 3, background: c }}/>)}
...
function TopNav({ active, onNav }) {
  ...
  return (
    <div style={{ background: "rgba(8,8,8,0.96)", backdropFilter: "blur(24px)", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", padding: "0 16px", position: "sticky", top: 0, zIndex: 100 }}>
      ...
      <div style={{ width: 32, height: 32, borderRadius: "50%", background: `linear-gradient(135deg, ${T.greenDeep}, ${T.green})`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 12, color: T.white, flexShrink: 0 }}>
        DR
      </div>
    </div>
  );
